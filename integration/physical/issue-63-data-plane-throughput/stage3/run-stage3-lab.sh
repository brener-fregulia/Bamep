#!/usr/bin/env bash
#
# Bamep Issue #63 Stage 3 — physical MiniPC throughput-matrix lab supervisor.
...
#
# =====================================================================
# THE PHYSICAL 36-CASE MATRIX IS ARMED ONLY BEHIND  --arm.
# Without --arm this prints "PHYSICAL MATRIX NOT ARMED" and exits 0.
# Even WITH --arm the supervisor first runs every host-side preflight and only
# then prints READY_FOR_MINIPC_POWER_ON. It NEVER powers the MiniPC and NEVER
# starts a physical source read or a transfer.
# =====================================================================
#
# One foreground supervisor that owns every background service the Stage-3
# matrix needs, composing ALREADY-PROVEN pieces:
#   * Stage 1  — derived Secure-Boot/iPXE/wimboot runtime + winpeshl auto-start
#                + SetSystemTime UTC alignment (via the Stage-1 runner in
#                --matrix --arm mode);
#   * Stage 2  — the deterministic 36-case matrix engine + coordinator + probe;
#   * Issue #61 CP7 shape — real PostgreSQL + AgentControlGateway/WSS +
#                first-contact credential + Worker HTTPS + Transfer/Artifact
#                lifecycle + terminal Artifact::Verified evidence
#                (a SEPARATE stage3-harness process; #61 is not modified).
#
# The intended experiment variable is chunk_size ONLY: 8/16/32/64 MiB, 2048 MiB
# extent per case, 4 warm-ups + 32 measured = 36 transfers, one WinPE boot for
# the whole matrix, NO reboot between cases, NO deliberate fault injection.
#
# Verification (no physical boot triggered by any of these):
#   bash -n run-stage3-lab.sh
#   ./run-stage3-lab.sh --preflight            (read-only gate; no services)
#   ./run-stage3-lab.sh --arm --preflight      (armed interface; still no boot)
#
# Owner action, AFTER READY_FOR_MINIPC_POWER_ON (ONLY this):
...
#   4. type NOTHING else in WinPE — the injected winpeshl.ini auto-runs the
#      whole 36-case matrix.
#
...
STAGE3_DIR="$(dirname "${SCRIPT_PATH}")"
I63_DIR="$(dirname "${STAGE3_DIR}")"
...
DERIVE="${STAGE3_DIR}/derive-stage3-runtime.sh"

STORAGE_ROOT="${I63_STORAGE_ROOT:-${I63_DIR}/stage3-harness/runtime-stage3/chunkstore}"
DB_NAME="bamep_physint_spike"

# 36 x 2 GiB preserved payload + margin.
MATRIX_PAYLOAD_BYTES=$((36 * 2147483648))     # 77309411328 = 72 GiB
MIN_FREE_BYTES=$((90 * 1024 * 1024 * 1024))   # 96636764160 = 90 GiB
...
# Stock-WinPE-proven DLL set (#60/#61 evidence). Both the runner AND the probe
# must import a SUBSET of this — a new name is a STOP-and-report condition.
...
RUN_ID="i63s3-$(date +%Y%m%dT%H%M%S)"
...
# Prefer llvm-objdump (from the rustup llvm-tools component) — GNU objdump 2.4x
# does not emit "DLL Name:" headers for these PEs. `PE_OBJDUMP` is resolved once.
...
pe_dlls_ok() {
  ...
  [ "${found}" -eq 1 ] || log "  warn: could not read PE imports of $2 (no working objdump) — inspect with llvm-readobj --coff-imports"
  return 0
}

db_reachable() {
  # Read-only reachability check; NEVER creates/migrates the DB.
...
usage() { sed -n '3,55p' "${SCRIPT_PATH}" | sed 's/^#\{1,\} \{0,1\}//;s/^#$//'; }
...
if [ "${ARMED}" -eq 0 ] && [ "${PREFLIGHT_ONLY}" -eq 0 ]; then
  echo "PHYSICAL MATRIX NOT ARMED"
  echo
  echo "run-stage3-lab.sh needs an explicit --arm to start the physical services."
  echo "  ./run-stage3-lab.sh --preflight        # read-only host checks, no services"
  echo "  ./run-stage3-lab.sh --arm --preflight  # armed interface, host checks only, still no boot"
  echo "  ./run-stage3-lab.sh --arm              # bring the lab up; prints READY_FOR_MINIPC_POWER_ON"
  exit 0
fi

# ---------------------------------------------------------------------
# 1. PREFLIGHT — read-only. No network mutation, no services.
# ---------------------------------------------------------------------
hr
log "Bamep Issue #63 Stage 3 matrix lab supervisor — ${RUN_ID}"
...
require_file "${DERIVE}" "derive-stage3-runtime.sh"
...
log "[preflight] deterministic 36-case matrix plan + chunk arithmetic + disk budget"
( cd "${I63_DIR}/coordinator" && "${COORD_BIN}" --matrix-selftest 2>&1 | tail -3 ) | tee -a "${LAUNCHER_LOG}" >&2
"${COORD_BIN}" --matrix-selftest >/dev/null 2>&1 || die "coordinator --matrix-selftest FAILED (plan does not validate)"
"${COORD_BIN}" --matrix 2>&1 | grep -q 'PHYSICAL MATRIX NOT ARMED' || die "coordinator --matrix (no --arm) must print PHYSICAL MATRIX NOT ARMED"
log "  ok: 36-case plan validates; payload=${MATRIX_PAYLOAD_BYTES} bytes (72 GiB); 8/16/32/64 MiB -> 256/128/64/32 chunks over 2048 MiB"

log "[preflight] Worker storage root + free space (>= 90 GiB for 36 preserved 2 GiB Artifacts)"
...
log "[preflight] runtime / credential directories writable + git-ignored"
for d in "${I63_DIR}/stage3-harness/runtime-stage3" "${I63_DIR}/evidence"; do
  mkdir -p "${d}"
...
if pgrep -af 'bamep-i63-stage3-harness|bamep-i63-stage1-coordinator --matrix' >/dev/null 2>&1; then
  log "  CONFLICT: a stale Issue-63 Stage-3 process is still running"; CONFLICT=1
fi
...
log "[preflight] scratch space for the derived runtime (~few MiB copied + symlinks)"
...
if [ "${PREFLIGHT_ONLY}" -eq 1 ]; then
  ...
  if [ "${ARMED}" -eq 1 ]; then
    ...
    log "started, NO derive, NO MiniPC boot. Re-run './run-stage3-lab.sh --arm' to bring the lab up."
  else
    log "PREFLIGHT_OK — no network state changed, no services started. Add --arm to run the matrix."
  fi
  hr
  exit 0
fi
...
on_signal() {
  ...
  if [ -f "${MATRIX_VERDICT}" ]; then
    log "Stage-3 matrix terminal: $(cat "${MATRIX_VERDICT}" 2>/dev/null) — lab shutting down (launcher exit ${rc})"
  elif [ -f "${WATCHDOG_ABORT}" ]; then
...
# ---------------------------------------------------------------------
# 4. #61-shaped Server/Postgres/WSS/Worker harness (fingerprint source)
# ---------------------------------------------------------------------
hr
log "STARTING #61-SHAPED STAGE-3 HARNESS (real Postgres + WSS + Worker HTTPS + terminal evidence)"
...
register_child "stage3-harness" "$!" 0
await "harness worker.https_listening" 120 grep -q '"event":"worker.https_listening"' "${HARNESS_LOG}" \
  || die "stage3-harness never reported worker.https_listening — see ${HARNESS_LOG}"
...
log "  harness leaf-cert SHA-256 fingerprint captured"
...
# ---------------------------------------------------------------------
# 6. derive the Issue-63 Stage-3 runtime (pin baked into the WinPE bootstrap)
# ---------------------------------------------------------------------
hr
log "DERIVING the Issue-63 Stage-3 PXE/WinPE runtime (Phase-9d consumed READ-ONLY)"
if ! "${DERIVE}" --out "${DERIVE_OUT}" --runner-exe "${RUNNER_EXE}" --probe-exe "${PROBE_EXE}" \
  ...
  die "derive-stage3-runtime.sh failed — see ${EVID}/derive.log"
fi
...
# ---------------------------------------------------------------------
# 7. WinPE HTTP + dnsmasq + matrix coordinator
# ---------------------------------------------------------------------
hr
...
log "[coordinator] bamep-i63-stage1-coordinator --matrix --arm (typed 36-case authority + probe sink)"
rm -f "${MATRIX_VERDICT}"
"${COORD_BIN}" --matrix --arm --matrix-addr "${LAB_IP}:${PORT_MATRIX}" --sink-addr "${LAB_IP}:${PORT_SINK}" \
  --evidence-dir "${MATRIX_EVID}" --run-id "${RUN_ID}" --verdict-file "${MATRIX_VERDICT}" \
  --observed-free-bytes "${FREE_BYTES}" > "${EVID}/coordinator.log" 2>&1 &
register_child "matrix-coordinator" "$!" 0
await "coordinator STAGE3_MATRIX_LISTENING" 30 grep -q '^STAGE3_MATRIX_LISTENING ' "${EVID}/coordinator.log" \
...
gate "tcp/${PORT_MATRIX} (matrix coord)"     tcp_up "${PORT_MATRIX}"
...
gate "matrix coordinator ARMED"              grep -q '^STAGE3_MATRIX_ARMED ' "${EVID}/coordinator.log"
gate "matrix disk budget OK"                 bash -c "! grep -q 'STAGE3_MATRIX_START_FAIL' '${EVID}/coordinator.log'"
...
for f in winpeshl.ini bamep-i63-stage3-bootstrap.cmd bamep-i63-runner.exe bamep-i63-stage2-probe.exe bamep-i63-enroll.cred; do
...
gate "autoexec injects stage3 bootstrap"    grep -qF "/bamep-i63-stage3-bootstrap.cmd bamep-i63-stage3-bootstrap.cmd" "${AX}"
...
gate "bootstrap.cmd launches runner --matrix --arm" grep -qF -- '--matrix --arm' "${DERIVE_OUT}/http/bamep-i63-stage3-bootstrap.cmd"
gate "bootstrap.cmd carries this fingerprint"       grep -qF "${FINGERPRINT}" "${DERIVE_OUT}/http/bamep-i63-stage3-bootstrap.cmd"
gate "matrix-plan.json written"             bash -c "[ -f '${MATRIX_EVID}/matrix-plan.json' ]"
  ...
  while :; do
    ...
    for k in "${!pids[@]}"; do
      if ! proc_alive "${pids[$k]}"; then
        if [ "${names[$k]}" = "matrix-coordinator" ] && [ -f "${MATRIX_VERDICT}" ]; then
          log ""
          log ">>> matrix coordinator reached its terminal verdict ($(cat "${MATRIX_VERDICT}" 2>/dev/null)) and exited (expected) — tearing the lab down"
        else
          log ""
...
Stage-3 harness: READY   real Postgres(${DB_NAME}) + WSS ${LAB_IP}:${PORT_WSS} + Worker HTTPS ${LAB_IP}:${PORT_DP} + coord ${LAB_IP}:${PORT_COORD}
Matrix coord   : READY   ARMED   (typed 36-case authority ${LAB_IP}:${PORT_MATRIX}  +  probe sink ${LAB_IP}:${PORT_SINK})
Phase-9d       : byte-identical before/after derive (7/7 pinned)
Storage root   : ${STORAGE_ROOT}   free=${FREE_BYTES} bytes  (>= 90 GiB gate; 72 GiB payload preserved)
Clock method   : SetSystemTime (UTC); strict skew window [${SKEW_FLOOR_MS}, ${SKEW_CEIL_MS}] ms; aligned OUTSIDE every measured wall

Matrix (fixed; the ONLY experiment variable is chunk_size):
  extent      2,147,483,648 bytes (2048 MiB) per case
  chunk sizes 8 / 16 / 32 / 64 MiB  ->  256 / 128 / 64 / 32 chunks (no partial final chunk)
  4 warm-ups (excluded) + 8 balanced cycles x 4 = 36 transfers, ONE WinPE boot, NO reboot between cases
  clean fast path: NO auth-denial, NO listener restart, NO deliberate retry/resume fault
...
Evidence: ${EVID}
  ...
  matrix/{matrix-plan.json, case-results.ndjson, coordinator-events.ndjson, probe-evidence.ndjson, analysis.json}
...
Owner action (ONLY this):
  ...
  3. Press a key ONCE at wimboot's "Press any key to continue booting..." prompt.
  4. Type NOTHING in WinPE. winpeshl.ini auto-runs bootstrap -> wpeinit -> runner --matrix --arm,
     which drives all 36 cases and reports each to the matrix coordinator.
Watch this terminal for  STAGE3_MATRIX_TERMINAL marker=matrix_pass  (or matrix_fail). Ctrl-C to tear down.
==================================================
EOF
...
(
  tail -n +1 -F "${EVID}/coordinator.log" 2>/dev/null | while IFS= read -r line; do
    printf '%s\n' "${line}"
    case "${line}" in
      STAGE3_MATRIX_TERMINAL*marker=matrix_pass*)
        printf '\n############################################\n# STAGE 3 MATRIX PASS — 36/36 verified — review the evidence\n# %s\n############################################\n' "${EVID}" ;;
      STAGE3_MATRIX_TERMINAL*marker=matrix_fail*)
        printf '\n!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n! STAGE 3 MATRIX FAILED — stopped, evidence preserved\n! %s\n!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n' "${EVID}" ;;
    esac
  done
) &
TAIL_PID=$!
...
log "Stage-3 lab supervise ended — launcher exit ${RC} (verdict: $(cat "${MATRIX_VERDICT}" 2>/dev/null || echo none))"
exit "${RC}"
