...
#
# Reverts every runtime change made by issue53-winpe-phase8-setup-run.sh
...
#
# Issue #53 Phase 8: PXE-authored BCD diagnostic, on the Phase 3 baseline
# (original media bootx64.efi, NOT Phase 7). Tests whether replacing the
# stock-media BCD with a PXE-authored BCD (Microsoft-documented PXE
# WinPE model) causes Windows Boot Manager to request a RAMDISK-related
# asset (boot.sdi/boot.wim), under the same physical Secure Boot state
# (diskless, Secure Boot Enabled/Active/Standard, factory keys
# restored). See issue53-winpe-phase8-setup-run.sh for the full context
...
echo "== Bamep Issue #53 Phase 8 - CLEANUP =="
