...
#
# Reverts every runtime change made by issue53-phase9a2-setup-run.sh and
...
#
# Issue #53 Phase 9a2: shim's exact requested fallback path /ipxe.efi
# satisfied with the already-pinned official snponly.efi bytes, one
# variable change from the physically-executed Phase 9a baseline. See
# issue53-phase9a2-setup-run.sh for full context and artifact provenance.
...
# Same discrimination logic as setup's Gate C2 (kept identical on purpose -
# this is the fix for the Phase 9a inconsistency, where cleanup used a bare
# port-only grep and warned about the already-known, benign Tailscale-only
# :443 listeners). A listener bound exclusively to some other specific
# address (Tailscale/management/loopback) is NOT a conflict and must not be
# reported as one; only a wildcard bind or a bind to ${ADDR_HOST} itself is.
...
echo "== Bamep Issue #53 Phase 9a2 - CLEANUP =="
...
if sudo pkill -INT -f "tcpdump .*${SPIKE_DIR}/pxe-shim-fallback.pcap" 2>/dev/null; then
...
if [ -d "${SPIKE_DIR}" ]; then
    ls -la "${SPIKE_DIR}"
    echo
    ls -la "${SPIKE_DIR}/tftp" 2>/dev/null
    echo
