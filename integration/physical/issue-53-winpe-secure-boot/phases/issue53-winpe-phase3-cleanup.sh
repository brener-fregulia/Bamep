...
#
# Reverts every runtime change made by issue53-winpe-phase3-setup-run.sh
...
#
# Issue #53 Phase 3: probes the next Windows network-boot boundary when
# the exact stock BCD (from the same retained media tree as bootx64.efi)
# is supplied at the literal path Windows Boot Manager requested in
# Phase 2, under the same physical Secure Boot state (diskless, Secure
# Boot Enabled/Active/Standard, factory keys restored). See
# issue53-winpe-phase3-setup-run.sh for the full context and artifact
# provenance.
...
echo "== Bamep Issue #53 Phase 3 - CLEANUP =="
