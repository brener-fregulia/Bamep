...
#
# Reverts every runtime change made by issue53-phase9c-setup-run.sh and
...
#
# Issue #53 Phase 9c: wimboot v2.9.0 + stock boot.wim, stopping at
# wimboot's own pause. See issue53-phase9c-setup-run.sh for full context
...
# Same discrimination logic as setup's Gate C2 (and Phase 9a/9a2/9b's
# corrected cleanup scripts). A listener bound exclusively to another
# specific address (Tailscale/management/loopback) is NOT a conflict;
# only a wildcard bind or a bind to ${ADDR_HOST} itself is.
...
echo "== Bamep Issue #53 Phase 9c - CLEANUP =="
echo "   Interface: ${IFACE}"
echo "   Spike dir: ${SPIKE_DIR}"
echo "   Purge evidence: ${PURGE}"
echo

echo "-- Stop only the Phase 9c HTTP server (matched by its serving directory) --"
if pkill -TERM -f "http.server .*--directory ${SPIKE_DIR}/http" 2>/dev/null; then
    echo "Stopped a running Phase 9c HTTP server."
else
    echo "No Phase 9c HTTP server running."
fi
echo

echo "-- Stop only the Phase 9c dnsmasq/tcpdump --"
if sudo pkill -INT -f "dnsmasq .*${SPIKE_DIR}/dnsmasq.conf" 2>/dev/null; then
    echo "Stopped a running Phase 9c dnsmasq."
else
    echo "No Phase 9c dnsmasq running."
fi
if sudo pkill -INT -f "tcpdump .*${SPIKE_DIR}/pxe-wimboot-pause.pcap" 2>/dev/null; then
    echo "Stopped a running Phase 9c tcpdump."
else
    echo "No Phase 9c tcpdump running."
fi
sleep 1
echo

echo "-- Confirm no Phase 9c listener remains on ${ADDR_HOST}:${HTTP_PORT} --"
