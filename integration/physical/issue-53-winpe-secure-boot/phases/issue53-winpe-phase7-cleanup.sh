...
#
# Reverts every runtime change made by issue53-winpe-phase7-setup-run.sh
...
#
# Issue #53 Phase 7: corrected rerun of the Phase 6 hypothesis. Phase 6
# staged WinSiPolicy.p7b at an incorrect EFI/Microsoft/Boot/Policies/
# path and therefore did not test the hypothesis ("harness prevented
# evaluation"). Phase 7 serves the SAME already-pinned artifact at the
# corrected path matching the literal wire RRQ - EFI/Microsoft/Boot/
# WinSiPolicy.p7b, no Policies/ component - on the Phase 6 harness
# otherwise unchanged (diskless, Secure Boot Enabled/Active/Standard,
# factory keys restored). See issue53-winpe-phase7-setup-run.sh for the
# full context and artifact provenance.
...
echo "== Bamep Issue #53 Phase 7 - CLEANUP =="
...
if [ -d "${SPIKE_DIR}" ]; then
    ...
    echo
    ls -la "${SPIKE_DIR}/tftp/EFI/Microsoft/Boot" 2>/dev/null
    echo
    echo "-- confirm the incorrect Phase 6 path is still absent --"
    if [ -e "${SPIKE_DIR}/tftp/EFI/Microsoft/Boot/Policies/WinSiPolicy.p7b" ]; then
        echo "WARNING: incorrect Phase 6 path exists in this evidence tree."
    else
        echo "OK: EFI/Microsoft/Boot/Policies/WinSiPolicy.p7b absent, as expected."
    fi
    echo
