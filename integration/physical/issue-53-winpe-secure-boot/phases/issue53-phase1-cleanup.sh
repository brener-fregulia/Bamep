...
#
# Reverts every runtime change made by issue53-phase1-setup-run.sh and
...
#
# Issue #53 Phase 1: rerun of the known-good #52 Run 2 chain under the
# owner-confirmed physical Secure Boot state (diskless, Secure Boot
# Enabled/Active/Standard, factory keys restored). See
# issue53-phase1-setup-run.sh for the full context.
...
echo "== Bamep Issue #53 Phase 1 - CLEANUP =="
...
if [ -d "${SPIKE_DIR}" ]; then
    ls -la "${SPIKE_DIR}"
    echo
    ls -la "${SPIKE_DIR}/tftp/EFI/fedora" 2>/dev/null
    echo
