...
#
# Reverts every runtime change made by issue53-phase9d-setup-run.sh and
...
#
# Issue #53 Phase 9d: wimboot v2.9.0 + external BCD + external boot.sdi +
# stock boot.wim, testing whether the physical Endpoint reaches a
# functional WinPE shell. See issue53-phase9d-setup-run.sh for full
# context and artifact provenance. This cleanup never touches the BCD/
# boot.sdi content itself - it only stops processes and reverts network
# state.
...
# Same discrimination logic as setup's Gate C2 (and every prior Phase 9
# sub-phase's corrected cleanup script). A listener bound exclusively to
# another specific address (Tailscale/management/loopback) is NOT a
# conflict; only a wildcard bind or a bind to ${ADDR_HOST} itself is.
...
echo "== Bamep Issue #53 Phase 9d - CLEANUP =="
echo "   Interface: ${IFACE}"
echo "   Spike dir: ${SPIKE_DIR}"
echo "   Purge evidence: ${PURGE}"
echo

echo "-- Stop only the Phase 9d HTTP server (matched by its serving directory) --"
if pkill -TERM -f "http.server .*--directory ${SPIKE_DIR}/http" 2>/dev/null; then
    echo "Stopped a running Phase 9d HTTP server."
else
    echo "No Phase 9d HTTP server running."
fi
echo

echo "-- Stop only the Phase 9d dnsmasq/tcpdump --"
if sudo pkill -INT -f "dnsmasq .*${SPIKE_DIR}/dnsmasq.conf" 2>/dev/null; then
    echo "Stopped a running Phase 9d dnsmasq."
else
    echo "No Phase 9d dnsmasq running."
fi
if sudo pkill -INT -f "tcpdump .*${SPIKE_DIR}/pxe-winpe-completion.pcap" 2>/dev/null; then
    echo "Stopped a running Phase 9d tcpdump."
else
    echo "No Phase 9d tcpdump running."
fi
sleep 1
echo

echo "-- Confirm no Phase 9d listener remains on ${ADDR_HOST}:${HTTP_PORT} --"
