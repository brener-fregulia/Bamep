//! PROVENANCE: byte-identical copy of
#![allow(dead_code)] // resolver.rs / sources.rs are copied from #61 probe7 verbatim
//! `integration/physical/issue-63-data-plane-throughput/stage2-probe/src/resolver.rs`
//! (itself a verbatim copy of the closed Issue #61 probe7). Re-copied for the
//! Issue #65 throughput-first Spike; neither #61 nor #63 is refactored to share
//! it. The opaque `(source_observation_id, agent_source_id)` -> local locator
//! resolver is exactly the same-boot epoch safety primitive #65 needs.
//!
//! Original provenance chain:
//! `integration/physical/issue-61-endpoint-capture-data-plane/probe7/src/resolver.rs`
//! (Issue #61, closed). Not modified. The opaque `(source_observation_id,
//! agent_source_id)` -> local locator resolver is exactly what Issue #63 Stage 2
//! needs; #61 is not refactored to share it.
//!
