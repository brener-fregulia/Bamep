#![allow(dead_code)] // copied CP4/CP5/CP6 module; Issue #63 Stage 2 uses a subset

//! PROVENANCE: copy of
//! `integration/physical/issue-63-data-plane-throughput/stage2-probe/src/sources.rs`
//! (itself an adapted copy of the closed Issue #61 probe7). Re-copied verbatim
//! for the Issue #65 throughput-first Spike. Neither #61 nor #63 is refactored
//! to share it. #65 uses `enumerate` + `RawReadSource::open` (GENERIC_READ only)
//! + the 3-IOCTL device-length agreement — nothing else.
//!
//! Original: `integration/physical/issue-61-endpoint-capture-data-plane/probe7/src/sources.rs`
//! (Issue #61, closed), minimally adapted. Read-only physical enumeration
//! (zero-access query IOCTLs), `RawReadSource::open` (GENERIC_READ only, never
//! GENERIC_WRITE), and the 3-IOCTL device-length agreement. #61 is not
//! refactored to share it. The non-Windows stub reports the disposable-device
//! evidence tuple so the Issue-63 source-safety predicate can be exercised on
//! the host.

//! Issue #61 CP4/CP5/CP7A — read-only physical source enumeration + raw byte
//! access. CP7A drives [`RawReadSource::read_bytes_at`] once per logical chunk
//! across the bounded prefix; the non-Windows stub is a deterministic
//! position-keyed pattern (see [`pattern_bytes`]).
//!
...
pub fn enumerate() -> SourceEpoch {
    ...
    SourceEpoch {
        observation_id: new_observation_id(),
        sources: vec![
            // Issue #63 adaptation: the host stub for the disposable SSD carries
            // the REAL evidence tuple (model/serial/length) so the Issue-63
            // source-safety predicate exercises its Accept path on the host.
            LocalSource {
...
/// Non-Windows (developer host) stand-in used ONLY by the CP6 host loopback
/// smoke. It never touches a real device: `read_at` returns a deterministic
/// SplitMix64 byte pattern so the full harness↔probe wiring (ActionDispatch,
/// auth grant, one chunk PUT, idempotent retry, no seal) can be exercised with
/// synthetic chunk bytes. The real physical read is Windows-only and was
/// proven separately by CP5.
#[cfg(not(windows))]
...
#[cfg(not(windows))]
fn pattern_bytes(offset: u64, len: u64) -> Vec<u8> {
    // CP7A edit vs the CP6 probe: the byte at absolute position `p` is a pure
    // function of `p`, so the concatenation of any partition of `[0, total)`
    // equals the whole deterministic source. This lets the single-pass stream
    // tests assert `rolling_digest == SHA-256(exact bounded source bytes)`
    // regardless of how the pass is chunked or how many times a chunk is
    // re-read for resume reconciliation.
...
impl RawReadSource {
    ...
    pub fn device_length(&self) -> DeviceLength {
        DeviceLength {
            get_length_info: Some(256_060_514_304),
            drive_geometry_ex: Some(256_060_514_304),
            storage_read_capacity: Some(256_060_514_304),
            bytes_per_sector: Some(512),
        }
    }
    pub fn read_at(&self, label: &str, offset: u64, len: u64, c: &mut Counters) -> RangeRead {
        c.data_read_count += 1;
        let bytes = pattern_bytes(offset, len);
        ...
        RangeRead {
            ...
            sha256_hex: d.iter().map(|b| format!("{b:02x}")).collect(),
            ok: bytes.len() as u64 == len,
            elapsed_ms: 0,
        }
    }
    pub fn read_bytes_at(&self, offset: u64, len: u64, c: &mut Counters) -> Result<Vec<u8>, String> {
        c.data_read_count += 1;
...
mod win {
    ...
    impl RawReadSource {
        ...
        /// CP6: read one bounded range and RETURN the exact bytes (for a chunk
        /// upload). Still `GENERIC_READ` only, still no write.
        pub fn read_bytes_at(&self, offset: u64, len: u64, c: &mut Counters) -> Result<Vec<u8>, String> {
            c.data_read_count += 1;
            let mut new_pos: i64 = 0;
            if unsafe { SetFilePointerEx(self.handle, offset as i64, &mut new_pos, FILE_BEGIN) } == 0
                || new_pos as u64 != offset
            {
                return Err(format!("SetFilePointerEx to {offset} failed"));
            }
            let mut out = Vec::with_capacity(len as usize);
            const CHUNK: usize = 1 << 20;
            let mut backing = vec![0u8; CHUNK + 4096];
            ...
            while (out.len() as u64) < len {
                let want = (len - out.len() as u64).min(CHUNK as u64) as u32;
                let mut got = 0u32;
                if unsafe {
                    ReadFile(self.handle, backing[pad..].as_mut_ptr(), want, &mut got, std::ptr::null_mut())
                } == 0
                {
                    return Err("ReadFile failed mid-range".into());
                }
                if got == 0 {
                    break;
                }
                out.extend_from_slice(&backing[pad..pad + got as usize]);
            }
            if out.len() as u64 != len {
                return Err(format!("short read: {} of {len}", out.len()));
            }
            Ok(out)
        }
    }
