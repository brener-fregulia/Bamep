...
/// `SOURCE_LEN` at `CHUNK_SIZE` spans three chunks (`4096 + 4096 + 1808`), so
/// progress carries genuine intermediate values and resume has real work.
pub const CHUNK_SIZE: u32 = 4096;
pub const SOURCE_LEN: usize = 10_000;
...
impl TempDir {
    pub fn fresh(tag: &str) -> Self {
        ...
        let dir = std::env::temp_dir().join(format!("bamep-rf005-{tag}-{}", Uuid::new_v4()));
...
impl TempSocketDir {
    fn fresh() -> Self {
        let dir = TempDir::fresh("uds");
        let path = dir.0.join("worker.sock");
        Self { _dir: dir, path }
    }
}
