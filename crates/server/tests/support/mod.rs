...
/// environment (it may use whatever authentication that environment needs,
/// including a password). With no override, a peer-authenticated DSN is
/// derived for the current OS user over the local PostgreSQL Unix socket —
/// no password, no `pg_hba.conf` change, no hard-coded username
/// (`docs/development/testing.md` "PostgreSQL integration tests"). The
/// derived DSN targets the always-present `postgres` maintenance database;
/// [`with_database`] swaps in each per-test name.
///
/// `$PGUSER` is intentionally *not* consulted: it names a PostgreSQL role,
/// and Unix-socket peer authentication requires the role to match the OS
/// user. A different role needs an explicit `BAMEP_TEST_PG_ADMIN_URL`.
fn admin_url() -> String {
    ...
    #[cfg(unix)]
    {
        ...
        let user = nonempty("USER")
            ...
            .unwrap_or_else(|| {
                panic!(
                    "cannot derive a PostgreSQL admin connection: neither $USER nor $LOGNAME \
                     is set.\n\
...
/// A non-secret description of a PostgreSQL DSN for diagnostics: scheme,
/// host, and database only. Userinfo (which may hold a password) and the
/// query string are dropped, so `BAMEP_TEST_PG_ADMIN_URL` is never surfaced
/// verbatim in a test failure message.
...
impl TestDatabase {
    ...
    pub async fn teardown(self) {
        ...
        let admin_pool = PgPool::connect(&self.admin_url)
            ...
            .unwrap_or_else(|e| {
                panic!(
                    "cannot reconnect to the admin PostgreSQL database `{}` for teardown: \
